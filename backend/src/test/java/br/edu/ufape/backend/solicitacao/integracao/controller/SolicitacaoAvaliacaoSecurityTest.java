...
class SolicitacaoAvaliacaoSecurityTest {
	...
	private String criarAvaliadorERetornarToken(String email) {
		usuarioContrato
				.salvar(new Avaliador("Avaliador Teste", email, "hash-senha", "REG-001", "Ciencia da Computacao"));
	...
	void avaliadorPodeAvaliarSolicitacao() throws Exception {
		...
		mockMvc.perform(patch("/api/v1/solicitacoes/" + solicitacao.getId() + "/avaliacao")
				...
				.content(objectMapper.writeValueAsString(Map.of("decisao", "APROVADA")))).andExpect(status().isOk())
				.andExpect(jsonPath("$.status").value("APROVADA")).andExpect(jsonPath("$.totalAtividades").value(0));
	}
	...
	void avaliadorPodeDetalharSolicitacaoDeQualquerEstudante() throws Exception {
		SolicitacaoValidacao solicitacao = criarSolicitacaoSubmetida();
	...
	@Test
	@DisplayName("ESTUDANTE autenticado deve receber 403 Forbidden em GET /{id}/avaliacao")
	void estudanteNaoPodeDetalharSolicitacaoParaAvaliacao() throws Exception {
		SolicitacaoValidacao solicitacao = criarSolicitacaoSubmetida();
