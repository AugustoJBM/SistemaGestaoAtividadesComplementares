...
class AtividadeControllerPutIntegrationTest {
	...
	void deveRetornar200AoEditarAtividadeComSucesso() throws Exception {
		AtividadeResponseDTO responseMock = new AtividadeResponseDTO(1L, "Título Atualizado", "UFAPE",
				LocalDate.of(2026, 5, 10), 20, Natureza.ACC, Categoria.ENSINO, LocalDateTime.of(2026, 5, 1, 10, 0),
				"estudante@ufape.edu.br");

		when(atividadeFacade.atualizarAtividade(eq(1L), any(AtualizarAtividadeRequestDTO.class), any(),
				eq("estudante@ufape.edu.br"))).thenReturn(responseMock);

		MockMultipartFile arquivo = new MockMultipartFile("arquivo", "novo_certificado.pdf",
				MediaType.APPLICATION_PDF_VALUE, "conteudo".getBytes());

		mockMvc.perform(MockMvcRequestBuilders.multipart(HttpMethod.PUT, "/api/v1/atividades/1").file(arquivo)
				.principal(new UsernamePasswordAuthenticationToken("estudante@ufape.edu.br", "password"))
				.param("titulo", "Título Atualizado").param("instituicaoResponsavel", "UFAPE")
				.param("dataRealizacao", "2026-05-10").param("cargaHoraria", "20").param("natureza", "ACC")
				.param("categoria", "ENSINO")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1L))
	...
	void deveRetornar400QuandoDadosForemInvalidos() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.multipart(HttpMethod.PUT, "/api/v1/atividades/1")
				.principal(new UsernamePasswordAuthenticationToken("estudante@ufape.edu.br", "password"))
				.param("titulo", "").param("cargaHoraria", "0").param("natureza", "").param("categoria", ""))
	...
	void deveRetornar401QuandoNaoAutenticado() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.multipart(HttpMethod.PUT, "/api/v1/atividades/1")
				.param("titulo", "Título Válido").param("dataRealizacao", "2026-05-10").param("cargaHoraria", "10")
				.param("natureza", "ACC").param("categoria", "ENSINO").param("instituicaoResponsavel", "UFAPE"))
	...
	void deveRetornar400QuandoInstituicaoResponsavelEstiverEmBranco(String instituicaoResponsavel) throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.multipart(HttpMethod.PUT, "/api/v1/atividades/1")
				.principal(new UsernamePasswordAuthenticationToken("estudante@ufape.edu.br", "password"))
				.param("titulo", "Título Válido").param("dataRealizacao", "2026-05-10").param("cargaHoraria", "15")
				.param("natureza", "ACC").param("categoria", "ENSINO")
				.param("instituicaoResponsavel", instituicaoResponsavel)).andExpect(status().isBadRequest())
	...
	void deveRetornar400QuandoDataRealizacaoEstiverNoFuturo() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.multipart(HttpMethod.PUT, "/api/v1/atividades/1")
				.principal(new UsernamePasswordAuthenticationToken("estudante@ufape.edu.br", "password"))
				.param("titulo", "Título Válido").param("dataRealizacao", LocalDate.now().plusDays(1).toString())
				.param("cargaHoraria", "10").param("natureza", "ACC").param("categoria", "ENSINO")
				.param("instituicaoResponsavel", "UFAPE")).andExpect(status().isBadRequest())
