...
class AtividadeControllerTest {
	...
	private String criarAvaliadorERetornarToken(String email) {
		usuarioContrato.salvar(new Avaliador("Avaliador Progresso", email, "hash-irrelevante", "REG-001", "Extensao"));
		return jwtService.generateToken(email);
	}
	...
	void deveRetornarProgressoDoEstudanteAutenticadoComSucesso() throws Exception {
		...
		mockMvc.perform(get(URL_PROGRESSO).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$.acc").exists())
				...
				.andExpect(jsonPath("$.acex").exists()).andExpect(jsonPath("$.acex.horasAcumuladas").exists())
	...
	void deveRetornarUnauthorizedQuandoNaoHouverToken() throws Exception {
		mockMvc.perform(get(URL_PROGRESSO)).andExpect(status().isUnauthorized())
				.andExpect(jsonPath("$.status").value(401)).andExpect(jsonPath("$.message").exists());
	}
	...
	void deveRetornarUnauthorizedQuandoTokenForInvalido() throws Exception {
		mockMvc.perform(get(URL_PROGRESSO).header(HttpHeaders.AUTHORIZATION, "Bearer token.invalido.malformado"))
	...
	void deveResponderComMensagemCoerenteComOContextoDeProgresso() throws Exception {
		...
		mockMvc.perform(get(URL_PROGRESSO).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isForbidden()).andExpect(result -> {
					String corpo = result.getResponse().getContentAsString();
					org.junit.jupiter.api.Assertions.assertFalse(corpo != null && corpo.contains("cadastro"),
	...
	void naoDeveExistirRotaComIdDeEstudanteNoPath() throws Exception {
		...
		mockMvc.perform(get(URL_PROGRESSO + "/999999").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
	...
	void deveIgnorarParametroDesconhecidoNaQueryString() throws Exception {
		...
		String respostaSemParametro = mockMvc
				.perform(get(URL_PROGRESSO).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();

		String respostaComParametroDeOutroEstudante = mockMvc.perform(
				get(URL_PROGRESSO).param("estudanteId", "999999").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
	...
	void deveDerivarEstudanteExclusivamenteDoTokenIgnorandoIdentidadeDeOutroEstudante() throws Exception {
		...
		String respostaA = mockMvc
				.perform(get(URL_PROGRESSO).header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenEstudanteA))
				.andExpect(status().isOk()).andExpect(jsonPath("$.acc").exists()).andExpect(jsonPath("$.acex").exists())
				.andReturn().getResponse().getContentAsString();

		String respostaAComIdDeB = mockMvc
				.perform(get(URL_PROGRESSO).param("estudanteId", "999999").header(HttpHeaders.AUTHORIZATION,
						"Bearer " + tokenEstudanteA))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();

		mockMvc.perform(get(URL_PROGRESSO).header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenEstudanteB))
				.andExpect(status().isOk()).andExpect(jsonPath("$.acc").exists())
