...
class AtividadeControllerAvaliacaoSecurityTest {
	...
	private Estudante criarEstudante(String email) {
		return (Estudante) usuarioContrato
				.salvar(new Estudante("Estudante Teste", email, "hash-senha", "2026001", "BCC"));
	}

	private String criarAvaliadorERetornarToken(String email) {
		usuarioContrato
				.salvar(new Avaliador("Avaliador Teste", email, "hash-senha", "REG-001", "Ciência da Computação"));
		return jwtService.generateToken(email);
	}

	private String criarAdminERetornarToken(String email) {
		usuarioContrato.salvar(new Administrador("Admin Teste", email, "hash-senha", "TOTAL", "Coordenação"));
		return jwtService.generateToken(email);
	}
	...
	private AtividadeComplementar criarAtividadePendente(Estudante estudante) {
		...
		AtividadeComplementar atividade = new AtividadeComplementar("Monitoria de Programação", "UFAPE",
				LocalDate.now(), 30, Natureza.ACC, Categoria.ENSINO, cert, estudante);
	...
	void estudanteNaoPodeAvaliarAtividade() throws Exception {
		...
		AvaliacaoDecisaoRequestDTO dto = new AvaliacaoDecisaoRequestDTO(DecisaoAvaliador.DEFERIDO,
				"Tentativa de autoavaliação");

		mockMvc.perform(post("/api/v1/atividades/" + atividade.getId() + "/avaliar")
				.header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenEstudante).contentType(MediaType.APPLICATION_JSON)
	...
	void avaliadorPodeAvaliarAtividade() throws Exception {
		...
		mockMvc.perform(post("/api/v1/atividades/" + atividade.getId() + "/avaliar")
				.header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenAvaliador).contentType(MediaType.APPLICATION_JSON)
	...
	void requisicaoSemAutenticacaoRetornaUnauthorized() throws Exception {
		AvaliacaoDecisaoRequestDTO dto = new AvaliacaoDecisaoRequestDTO(DecisaoAvaliador.DEFERIDO, "Sem token");

		mockMvc.perform(post("/api/v1/atividades/1/avaliar").contentType(MediaType.APPLICATION_JSON)
