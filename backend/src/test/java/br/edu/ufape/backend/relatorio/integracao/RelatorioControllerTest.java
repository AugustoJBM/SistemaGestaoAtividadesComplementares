package br.edu.ufape.backend.relatorio.integracao;
...
class RelatorioControllerTest {
	...
	private String criarAvaliadorERetornarToken(String email) {
		usuarioContrato.salvar(new Avaliador("Avaliador Relatorio", email, "hash-irrelevante", "REG-001", "Extensao"));
		return jwtService.generateToken(email);
	}
	...
	void deveRetornarRelatorioVazioParaEstudanteSemAtividades() throws Exception {
		...
		mockMvc.perform(get(URL_RELATORIO).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.estudanteEmail").value("relatorio.vazio@ufape.edu.br"))
				.andExpect(jsonPath("$.naturezas").isArray()).andExpect(jsonPath("$.naturezas").isEmpty())
				.andExpect(jsonPath("$.totalHorasAcc").value(0)).andExpect(jsonPath("$.totalHorasAcex").value(0))
	...
	void deveRetornarRelatorioAgrupadoParaEstudanteComAtividades() throws Exception {
		...
		mockMvc.perform(get(URL_RELATORIO).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$.estudanteEmail").value(email))
				...
				.andExpect(jsonPath("$.naturezas[0].categorias[0].atividades[0].titulo")
						.value("Minicurso de Extensao ACC"))
				...
				.andExpect(
						jsonPath("$.naturezas[1].categorias[0].atividades[0].titulo").value("Evento de Extensao ACEX"))
				.andExpect(jsonPath("$.naturezas[1].categorias[0].atividades[0].cargaHorariaEmHoras").value(5))
				.andExpect(jsonPath("$.totalHorasAcc").value(8)).andExpect(jsonPath("$.totalHorasAcex").value(5))
	...
	void deveRetornarUnauthorizedQuandoNaoHouverToken() throws Exception {
		mockMvc.perform(get(URL_RELATORIO)).andExpect(status().isUnauthorized())
	...
	void avaliadorContinuaRecebendo403EmAtividades() throws Exception {
		...
		mockMvc.perform(get(URL_ATIVIDADES).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isForbidden());
	}
}
