...
class AtividadeControllerListagemTest {
	...
	private String criarAvaliadorERetornarToken(String email) {
		usuarioContrato.salvar(new Avaliador("Avaliador Listagem", email, "hash-irrelevante", "REG-001", "Extensao"));
		return jwtService.generateToken(email);
	}
	...
	private void cadastrarAtividade(String token, String titulo, Natureza natureza, Categoria categoria)
		...
		mockMvc.perform(multipart(URL_LISTAGEM).file(arquivo).param("titulo", titulo)
				.param("instituicaoResponsavel", "UFAPE").param("dataRealizacao", LocalDate.now().toString())
				.param("cargaHoraria", "8").param("natureza", natureza.name()).param("categoria", categoria.name())
	...
	void estudanteAutenticadoSemAtividadesRetorna200ComArrayVazio() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(get(URL_LISTAGEM).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
	...
	void estudanteComAtividadesRetorna200ComAtividadesDele() throws Exception {
		...
		// Act & Assert: garante isolamento por identidade do JWT, nao por stub
		// artificial
		mockMvc.perform(get(URL_LISTAGEM).header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenDono))
	...
	void filtroPorNaturezaFuncionaViaQueryParam() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(get(URL_LISTAGEM).param("natureza", "ACC").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
	...
	void filtroPorCategoriaFuncionaViaQueryParam() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(
				get(URL_LISTAGEM).param("categoria", "PESQUISA").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(1))
	...
	void naturezaInvalidaRetorna400() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(
				get(URL_LISTAGEM).param("natureza", "XPTO").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
	...
	void requisicaoSemTokenRetorna401() throws Exception {
		// Act & Assert
		mockMvc.perform(get(URL_LISTAGEM)).andExpect(status().isUnauthorized());
	}
