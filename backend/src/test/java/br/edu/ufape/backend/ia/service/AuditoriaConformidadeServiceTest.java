...
class AuditoriaConformidadeServiceTest {
	...
	void deveAuditarAtividadeEGravandoParecer() {
		ParecerResponseDTO dtoMock = new ParecerResponseDTO(null, null, "ACC", "ENSINO", 30, "Art. 12",
		...
		ParecerConformidade resultado = auditoriaService.auditarOuObterParecer(atividade);

		assertNotNull(resultado);
		assertEquals("ACC", resultado.getNaturezaSugerida());
		assertEquals("Art. 12", resultado.getArtigoRegulamento());
		...
		verify(iaContrato, times(1)).gerarParecerConformidade("Monitoria de Programação", "UFAPE", "ACC", "ENSINO", 30);
	}
}
