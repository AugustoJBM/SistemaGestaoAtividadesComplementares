...
class AtividadeProgressoControllerTest {
	...
	void deveRetornarProgressoComSucesso() throws Exception {
		ProgressoResponseDTO dto = new ProgressoResponseDTO(new ProgressoModalidadeResponseDTO(60, 0, 90),
		...
		mockMvc.perform(get("/api/v1/atividades/progresso")
				...
				.andExpect(status().isOk()).andExpect(jsonPath("$.acc.horasAcumuladas").value(60))
	...
	void deveRetornarUnauthorized() throws Exception {
		mockMvc.perform(get("/api/v1/atividades/progresso")).andExpect(status().isUnauthorized());
	}
}
