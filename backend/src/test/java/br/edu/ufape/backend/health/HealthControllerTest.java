...
class HealthControllerTest {
	...
	void deveRetornarBancoUpQuandoConexaoForValida() throws Exception {
		...
		mockMvc.perform(get("/api/v1/health/database")).andExpect(status().isOk())
				.andExpect(jsonPath("$.status").value("UP")).andExpect(jsonPath("$.database").value("UP"));
	}
	...
	void deveRetornarBancoDownQuandoConexaoInvalida() throws Exception {
		...
		mockMvc.perform(get("/api/v1/health/database")).andExpect(status().isServiceUnavailable())
				.andExpect(jsonPath("$.status").value("DOWN")).andExpect(jsonPath("$.database").value("DOWN"));
	}
