package br.edu.ufape.backend.solicitacao.integracao.controller;
...
class SolicitacaoEstudanteControllerTest {
	...
	private String criarAvaliadorERetornarToken(String email) {
		usuarioContrato.salvar(new Avaliador("Avaliador Teste", email, "hash-irrelevante", "REG-001", "Extensao"));
		return jwtService.generateToken(email);
	}
	...
	private void cadastrarAtividade(String token, String titulo, Natureza natureza, Categoria categoria,
		...
		mockMvc.perform(multipart(URL_ATIVIDADES).file(arquivo).param("titulo", titulo)
				.param("instituicaoResponsavel", "UFAPE").param("dataRealizacao", LocalDate.now().toString())
				.param("cargaHoraria", String.valueOf(cargaHoraria)).param("natureza", natureza.name())
				.param("categoria", categoria.name()).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
	...
	void deveSubmeterSolicitacaoComSucesso() throws Exception {
		...
		mockMvc.perform(post(URL_SOLICITACOES).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isCreated()).andExpect(jsonPath("$.id").exists())
				.andExpect(jsonPath("$.status").value("SUBMETIDA")).andExpect(jsonPath("$.dataSubmissao").exists())
				.andExpect(jsonPath("$.itens", hasSize(2))).andExpect(jsonPath("$.itens[0].atividadeId").exists())
	...
	void deveListarSolicitacoesDoEstudante() throws Exception {
		...
		mockMvc.perform(get(URL_SOLICITACOES).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(1))).andExpect(jsonPath("$[0].id").exists())
	...
	void deveDetalharSolicitacaoComSucesso() throws Exception {
		...
		MvcResult result = mockMvc.perform(post(URL_SOLICITACOES).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
		...
		mockMvc.perform(
				get(URL_SOLICITACOES + "/" + solicitacaoId).header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$.id").value(solicitacaoId))
				.andExpect(jsonPath("$.status").value("SUBMETIDA")).andExpect(jsonPath("$.dataSubmissao").exists())
				.andExpect(jsonPath("$.dataAvaliacao").value(nullValue()))
				.andExpect(jsonPath("$.justificativa").value(nullValue())).andExpect(jsonPath("$.itens", hasSize(2)))
	...
	void deveRetornar404AoAcessarSolicitacaoDeOutroEstudante() throws Exception {
		...
		MvcResult result = mockMvc
				.perform(post(URL_SOLICITACOES).header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenDono))
				.andExpect(status().isCreated()).andReturn();
		...
		mockMvc.perform(
				get(URL_SOLICITACOES + "/" + solicitacaoId).header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenOutro))
	...
	void deveRetornar404ParaSolicitacaoInexistente() throws Exception {
		...
		mockMvc.perform(get(URL_SOLICITACOES + "/999999").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isNotFound()).andExpect(jsonPath("$.status").value(404))
				.andExpect(jsonPath("$.message").value("Solicitação não encontrada."));
	}
	...
	void deveRetornar403ParaAvaliador() throws Exception {
		...
		mockMvc.perform(get(URL_SOLICITACOES + "/1").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
				.andExpect(status().isForbidden());
	}
	...
	void deveRetornar401ParaAnonimo() throws Exception {
		mockMvc.perform(post(URL_SOLICITACOES)).andExpect(status().isUnauthorized())
				.andExpect(jsonPath("$.status").value(401));

		mockMvc.perform(get(URL_SOLICITACOES)).andExpect(status().isUnauthorized())
				.andExpect(jsonPath("$.status").value(401));

		mockMvc.perform(get(URL_SOLICITACOES + "/1")).andExpect(status().isUnauthorized())
				.andExpect(jsonPath("$.status").value(401));
	}
}
