...
public class AuditoriaConformidadeService {
	...
	public AuditoriaConformidadeService(IaContrato iaContrato, ParecerConformidadeRepository parecerRepository) {
	...
	public ParecerConformidade auditarOuObterParecer(AtividadeComplementar atividade) {
		return parecerRepository.findByAtividadeId(atividade.getId()).orElseGet(() -> {
			try {
				...
				ParecerResponseDTO dto = iaContrato.gerarParecerConformidade(atividade.getTitulo(),
						atividade.getInstituicaoResponsavel(), atividade.getNatureza().name(),
						atividade.getCategoria().name(), atividade.getCargaHorariaEmHoras());
				...
				log.info(
						...
						atividade.getId());
				return parecerRepository.findByAtividadeId(atividade.getId()).orElseThrow(() -> ex);
			}
		});
	}
