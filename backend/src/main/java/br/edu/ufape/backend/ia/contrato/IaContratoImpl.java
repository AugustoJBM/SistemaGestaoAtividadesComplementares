...
public class IaContratoImpl implements IaContrato {
	...
	public IaContratoImpl(GroqRagService groqRagService, RegulamentoChunkRepository regulamentoChunkRepository,
	...
	public ExtracaoCertificadoResponseDTO extrairDadosArquivo(MultipartFile arquivo) {
		try {
			...
			return new ExtracaoCertificadoResponseDTO("Atividade Complementar", "Instituição Emissora",
					LocalDate.now(ZoneId.of("America/Recife")), 20, "ACC", "EVENTOS");
		}
	}
	...
	public ParecerResponseDTO gerarParecerConformidade(String titulo, String instituicao, String natureza,
		...
		return groqRagService.gerarParecerComContextoRAG(titulo, instituicao, natureza, categoria, cargaHoraria,
				contextoRecuperado);
	}
	...
	private String recuperarArtigosMaisRelevantes(String consulta) {
		try {
			...
			return todos.stream()
					...
					.limit(2).map(c -> c.getArtigo() + ": " + c.getConteudoTexto()).collect(Collectors.joining("\n\n"));
		} catch (Exception e) {
	...
	private float[] converterStringParaFloatArray(String str) {
		...
		for (int i = 0; i < partes.length; i++) {
			try {
				...
				log.trace("Valor nao numerico no vetor de embedding: {}", partes[i]);
			}
		}
		return vet;
	}
