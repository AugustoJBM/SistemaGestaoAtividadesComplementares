...
public class ProgressoService {
	...
	private int calcularHoras(Estudante estudante, Natureza natureza, boolean aprovadas) {
		List<AtividadeComplementar> atividades = atividadeComplementarRepository.findByEstudanteAndNatureza(estudante,
				natureza);
		return atividades.stream().filter(Objects::nonNull)
				.filter(a -> aprovadas ? RegraAtividadeValida.isAprovada(a) : RegraAtividadeValida.isPendente(a))
				.mapToInt(a -> a.getCargaHorariaEmHoras() != null ? a.getCargaHorariaEmHoras() : 0).sum();
	}
}
