...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	public void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (!jwtService.isTokenValid(token) || tokenBlacklistService.isTokenBlacklisted(token)) {
			escreverErroUnauthorized(response, "Token inválido, expirado ou revogado");
			return;
		}
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			try {
				...
				escreverErroUnauthorized(response, "Usuário do token não encontrado");
				return;
			}
		}
	...
	private void escreverErroUnauthorized(HttpServletResponse response, String mensagem) throws IOException {

		response.setStatus(HttpStatus.UNAUTHORIZED.value());
		response.setContentType(MediaType.APPLICATION_JSON_VALUE);
		response.setCharacterEncoding(StandardCharsets.UTF_8.name());

		ErroResponse erro = new ErroResponse(mensagem, HttpStatus.UNAUTHORIZED.value());
