...
public interface SolicitacaoValidacaoRepository extends JpaRepository<SolicitacaoValidacao, Long> {
	...
	boolean existsByAtividadeIdAndStatusIn(@Param("atividadeId") Long atividadeId,
	...
	@Query("SELECT new br.edu.ufape.backend.solicitacao.dto.SolicitacaoResumoResponseDTO("
			...
			+ "FROM SolicitacaoValidacao s LEFT JOIN s.itens item " + "WHERE s.estudanteId = :estudanteId "
			+ "GROUP BY s.id, s.status, s.dataSubmissao, s.dataAvaliacao " + "ORDER BY s.dataSubmissao DESC")
	List<SolicitacaoResumoResponseDTO> findResumosByEstudanteIdOrderByDataSubmissaoDesc(
	...
	@Query("SELECT DISTINCT s FROM SolicitacaoValidacao s LEFT JOIN FETCH s.itens "
			+ "WHERE (:status IS NULL OR s.status = :status) " + "ORDER BY s.dataSubmissao DESC, s.id DESC")
