...
@Component({
  ...
  template: `
    <a
      ...
      [attr.aria-label]="
        naoLidas() > 0 ? naoLidas() + ' notificações não lidas' : 'Nenhuma notificação não lida'
      "
    >
      <svg
      ...
      @if (naoLidas() > 0) {
        <span
          class="absolute top-1 right-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white transform translate-x-1/4 -translate-y-1/4 rounded-full bg-[#ba1a1a]"
        >
