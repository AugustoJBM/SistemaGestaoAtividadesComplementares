...
export class RelatorioService {
  ...
  private traduzirErro(error: HttpErrorResponse): string {
    ...
    if (error.status === 403) {
      return (
        this.mensagemDoBackend(error) ?? 'Apenas estudantes podem emitir o relatório de atividades.'
      );
    }
    return (
      this.mensagemDoBackend(error) ?? 'Não foi possível carregar seu relatório. Tente novamente.'
    );
  }
